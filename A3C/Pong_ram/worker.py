...
class Worker(object):
    ...
    """
    def __init__(self, name, env, policy_net, value_net, global_counter, discount_factor=0.99, summary_writer=None, max_global_steps=None, log_every_n=10000):
        self.name = name
        ...
        self.env = env
        self.log_every_n_step = log_every_n
        
        # Create local policy/value nets that are not updated asynchronously
        with tf.variable_scope(name):
            self.policy_net = PolicyEstimator(policy_net.input_shape, policy_net.num_outputs)
            self.value_net = ValueEstimator(policy_net.input_shape, reuse=True)
    ...
    def run_n_steps(self, n, sess):
        ...
        for _ in range(n):
            ...
            if local_t % self.log_every_n_step == 0:
