...
def test_fn():
    ...
    V = ValueEstimator(input_shape, reuse=True)
...
def build_shared_network(state, add_summaries=False):
    """
    Build a network fc1 -> fc2.
    The network is shared by both policy and value net.
    """
    #TODO should I use a relu?
    ...
    if add_summaries:
        tf.contrib.layers.summarize_activation(hid)
        tf.contrib.layers.summarize_activation(fc1)
...
class PolicyEstimator():
    ...
    def __init__(self, input_shape, num_outputs, reuse=False, trainable=True):
        self.input_shape = input_shape
        self.num_outputs = num_outputs
        
        # Placeholders
        self.states = tf.placeholder(shape=[None] + list(input_shape), dtype=tf.uint8, name="states")
        ...
        with tf.variable_scope("policy_net"):
            ...
            # Get the predictions for the chosen actions
            gather_indices = tf.range(batch_size) * tf.shape(self.probs)[1] + self.actions
            self.picked_action_probs = tf.gather(tf.reshape(self.probs, [-1]), gather_indices)
            
            #todo ignore entropy for now
...
class ValueEstimator():
    ...
    def __init__(self, input_shape, reuse=False, trainable=True):
        self.input_shape = input_shape
        ...
        with tf.variable_scope("value_net"):
            ...
            tf.summary.scalar(self.loss.name, self.loss) #todo error?
