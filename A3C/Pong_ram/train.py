...
ffmpeg_path = '/Users/michaelwong/anaconda/lib/python3.6/site-packages/cv2/'

if import_path not in sys.path:
    sys.path.append(import_path)
if ffmpeg_path not in sys.path:
...
tf.flags.DEFINE_string("env", "Pong-ram-v0", "Name of gym Atari environment, e.g. Breakout-v0")
...
env_ = make_env()
# todo add the use of VALID_ACTIONS
"""
...
"""
if len(env_.observation_space.shape) == 1:
    # This means we are running on low-dimensional observations (e.g. RAM)
    input_shape = env_.observation_space.shape
else:
    img_h, img_w, img_c = env_.observation_space.shape
...
with tf.device("/cpu:0"):
    ...
    with tf.variable_scope("global") as vs:
        policy_net = PolicyEstimator(input_shape, num_actions)
        value_net = ValueEstimator(input_shape, reuse=True)
    ...
    for worker_id in range(NUM_WORKERS):
        ...
        worker = Worker(
            ...
            max_global_steps=FLAGS.max_global_steps) # todo warning
